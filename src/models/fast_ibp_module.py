...
class FastIBP(BaseBoundedModulePL):
    ...
    def model_step(self, batch: Any):
        ...
        eps_scalar = self.eps_scheduler.get_eps(step=step)
        # Note: just for ablation study
        # eps_scalar = self.hparams.max_eps
        ##############################

        eps = eps_scalar * torch.ones_like(y).float().detach()

        loss_rob = torch.tensor(0.0).to(x.device)
        loss_tight = torch.tensor(0.0).to(x.device)
        loss_relu = torch.tensor(0.0).to(x.device)

        reg = epoch < self.hparams.schedule_start + self.hparams.schedule_length - 1
        robust = eps_scalar >= 1e-50

        if self.hparams.min_eps_for_regularizers is not None and reg and not robust:
        ...
        if reg:
            ...
            loss += (1.0 - eps_scalar / self.hparams.max_eps) * (
        ...
        if self.hparams.weight_decay_l1 > 0.0:
            ...
            loss += loss_l1
        return loss, preds, y, eps, loss_stn, loss_rob, loss_tight, loss_relu
